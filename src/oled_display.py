...
def display_home():
    ...
    temperature_liquid = temperatures.get("liquid")
    temperature_air = temperatures.get("air")
    if temperature_liquid is None or temperature_liquid < -99.0 or temperature_liquid > 200.0:
        oled.text(f"LIQ: N/A C", 4, 3 * (8 + 2), 0xFFFF)
    else:
        oled.text(f"LIQ: {temperature_liquid:.1f} C", 4, 3 * (8 + 2), 0xFFFF)
    if temperature_air is None or temperature_air < -99.0 or temperature_air > 200.0:
        oled.text(f"AIR: N/A C", 4, 4 * (8 + 2), 0xFFFF)
    else:
        oled.text(f"AIR: {temperature_air:.1f} C", 4, 4 * (8 + 2), 0xFFFF)
    oled.show()
